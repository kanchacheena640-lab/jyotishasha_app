...
import 'package:jyotishasha_app/features/kundali/widgets/yog_dosh_card.dart';

import '../../../helpers/test_harness.dart';

void main() {
  testWidgets(
    'renders Name → Effect → tappable "View Full Free Report →" when '
    'AVAILABLE, and calls onViewReport when tapped',
    (tester) async {
      var tapped = false;
      await tester.pumpTestHarness(
        Scaffold(
          body: YogDoshCard(
            name: 'Gajakesari Yog',
            effect: 'Brings wisdom, respect, and prosperity.',
    ...
    (tester) async {
      await tester.pumpTestHarness(
        const Scaffold(
          body: YogDoshCard(
            name: 'Mangal Dosh',
            effect: 'May cause friction in marital harmony.',
  ...
  testWidgets('uses the premium card shell — white, 18dp radius, soft '
    ...
    await tester.pumpTestHarness(
      const Scaffold(
        body: YogDoshCard(
          name: 'Kaalsarp Dosh',
          effect: 'Indicates obstacles that build long-term resilience.',
          status: ReportAvailabilityStatus.available,
          isHindi: false,
        ),
      ),
      ...
      const Scaffold(
        body: Column(
          children: [
            YogDoshCard(
              name: 'गजकेसरी योग',
              effect: 'बुद्धि और समृद्धि लाता है।',
              status: ReportAvailabilityStatus.available,
              isHindi: true,
            ),
            YogDoshCard(
              name: 'मंगल दोष',
              effect: 'वैवाहिक जीवन में चुनौती ला सकता है।',
              status: ReportAvailabilityStatus.comingSoon,
              isHindi: true,
            ),
          ],
